...
inputString = """una bokpr ftz ryw nau yknf fguaczl anu
...
valid = 0
words = list(map(lambda x: x.split(" "), inputString.split("\n")))
for line in words:
    validPassword = True
    wordsAlready = set()
    for word in line:
        hashes = frozenset(Counter(word).items())
        if hashes in wordsAlready:
            validPassword = False
            break
        wordsAlready.add(hashes)
